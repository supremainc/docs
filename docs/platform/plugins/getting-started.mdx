...
- **기타**: ASP.NET Core, Ruby on Rails 등

### 인증서 파일 준비

Session Bridge를 사용하는 경우, **BioStar X Service Manager**에서 다운로드한 공개키 파일이 필요합니다.

:::tip
구체적인 개발 환경 구축 방법과 코드 예시는 [개발 가이드](./development-guide)와 [SessionBridge 예제](./session-bridge-example)에서 확인하세요.
:::