...
BioStar X와 연동되는 플러그인을 개발하려면 다음 두 개의 엔드포인트를 반드시 구현해야 합니다. 이 문서에서는 `/bsx`를 예시 경로로 사용합니다.
...
사용자가 BioStar 웹 인터페이스에서 플러그인 아이콘을 클릭할 때 호출되는 엔드포인트입니다. 이 과정에서 BioStar에 로그한 사용자의 세션(session)을 요청해 응답을 받습니다.
