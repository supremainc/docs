...
**BioStar 2**를 시작하기 전에 알아야 할 내용을 안내합니다. **BioStar 2**는 32비트 운영 체제와 64비트 운영 체제를 지원합니다. **BioStar 2**를 설치하려는 PC의 시스템 사양을 확인한 후 설치하세요.

## 설치 전 참고 사항 {#before}

**BioStar 2**를 설치하기 전에 다음 사항을 확인하고 설치를 진행하세요.
...
* **BioStar 2** 최신 버전으로의 업그레이드는 **2.6.0** 버전부터 지원됩니다. **BioStar 2.6.0** 버전 이하에서 최신 버전으로 업그레이드하려면 다음 차례로 설치하세요.
...
* MS SQL Server와 **BioStar 2**가 서로 다른 PC에 설치되어 있는 경우 **BioStar 2**를 설치한 PC에 Native Client를 설치해야 합니다. [Link 4](https://www.microsoft.com/ko-kr/download/details.aspx?id=50402)

* MS SQL Server를 사용할 경우 서버 및 각 테이블의 `Collation` 옵션을 CI(Case-insensitive)로 설정하세요.

* 기존 버전의 **BioStar 2**에서 데이터베이스를 백업할 때 모든 서비스를 사용 중지한 후 진행하세요. 또한, AC 데이터베이스와 TA 데이터베이스를 함께 백업하고, 복원하지 않으면 TA 데이터베이스를 사용할 수 없습니다.
...
* **BioStar 2**가 사용하는 포트의 기본 값은 아래와 같습니다. 다른 프로그램에서 동일한 포트를 점유한다면, **BioStar 2**가 정상 구동되지 않을 수 있습니다.
...
**BioStar 2**를 설치하기 전에 사용자가 직접 구성한 데이터베이스를 사용한다면 아래 사항을 확인하세요.
...
#### Windows Authentication 데이터베이스 설정하기
...
* **BioStar 2**를 설치하기 위한 [시스템의 최소 사양](system-requirements)을 알아보세요. 

* **BioStar 2** 버전에 따라 추가된 기능과 호환되는 주변 장치의 펌웨어 호환 정보를 [다음 문서](product-compatibility)에서 확인하세요.

* **BioStar 2**를 빠르게 설치하는 방법은 [다음 문서](express-install)에서 확인할 수 있습니다.