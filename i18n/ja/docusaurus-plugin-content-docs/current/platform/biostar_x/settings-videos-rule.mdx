...
* ビデオルール機能は、**エッセンシャル**（Essential）ライセンス以上で追加オプションとして利用できます。ライセンスポリシーの詳細については、<DocLink docId='platform/biostar_x/licensing#features-add-ons' />を参照してください。

* 出入口のドアとカメラを連携する際には、そのカメラが所属するグループの権限が必要です。カメラグループの権限設定の詳細については <DocLink docId='platform/biostar_x/settings-videos-camera#addCameraPermission' />ご参照ください。
...
<Cmd sid='video.addRule.label.videoLog' product='2' /> オプションを有効にすると、モニタリング画面のイベントパネルのイベント項目にビデオログ（<IcVidLog />）アイコンが表示されます。ビデオログアイコンが表示されたイベント項目をクリックすると、画面右側に詳細情報が表示されます。特定のイベントが発生した時刻の映像を確認できます。
...
* ブックマークおよびイベントをVMSサーバーに送信する際、メッセージの言語を設定できます。詳しくは<DocLink docId='platform/biostar_x/settings-video-integration#vms-event-bookmark' />をご参照ください。
...
* カメラグループの権限を持たない管理者は、特定のカメラグループに属するカメラを表示または選択できません。カメラグループの権限設定の詳細については <DocLink docId='platform/biostar_x/settings-videos-camera#addCameraPermission' />ご参照ください。

* 1つのドアには最大4台のカメラを追加できます。

* 1つのルールに含まれたドアは、他のルールでは使用できません。

* カメラを複数のドアに制限なく連携でき、既に他のドアと連携されているカメラも再利用できます。

* ドアとカメラの連携ルールを削除するには、リストで削除したい項目のチェックボックスをクリックし、右上の <IcTrash2 /> <Cmd sid='button.delete' product='2' /> ボタンをクリックします。

* ドアと連携されたカメラは、<Cmd sid='shell.header.arena' /> ページのドアリストでドアのサブツリーとして確認できます。詳しくは<DocLink docId='platform/biostar_x/check-door-status#door-tree-structure' />をご参照ください。
...
ルール編集画面の各セクションの詳細については<DocLink inner='#add-camera-rule' />をご参照ください。
