...
* ビデオルール機能は、**アドバンス**（Advanced）ライセンス以上で追加オプションとして利用できます。 ライセンス ポリシーの詳細については、[次のドキュメント](licensing#features-add-ons) を参照してください。

* 出入口のドアとカメラを連携する際には、そのカメラが所属するグループの権限が必要です。 カメラグループの権限設定の詳細については、[次のドキュメント](settings-videos-camera#addCameraPermission)を参照してください。
...
* ブックマークおよびイベントをVMSサーバーに送信する際、メッセージの言語を設定できます。 詳細については[次のドキュメント](settings-video-integration#vms-event-bookmark)を参照してください。
...
* カメラグループの権限を持たない管理者は、特定のカメラグループに属するカメラを表示または選択できません。 カメラグループの権限設定の詳細については、[次のドキュメント](settings-videos-camera#addCameraPermission)を参照してください。
