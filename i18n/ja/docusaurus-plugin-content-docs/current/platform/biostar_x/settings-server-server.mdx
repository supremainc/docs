...
2. 画面左のサイドバーで <Cmd sid='setting.menu.system.root' /> → <Cmd sid="setting.menu.server.server" /> をクリック。
...
<Cmd sid="setting.server.logUploadType" product='2' /> オプションを <Cmd sid='common.auto' product='2' /> に設定すると、サーバーと装置間の接続が切断され、再接続時に装置に保存されたイベントログがサーバーに自動アップロードされます。
...
<Cmd sid="setting.server.automaticUserSync" product='2' /> オプションでは、サーバーとデバイス間でユーザー情報を同期する方法を選択できます。同期にはユーザーに割り当てられたアクセスレベルとアクセスグループも含まれるため、**BioStar X**サーバーでアクセスレベルまたはアクセスグループを変更すると、システムはその変更を関連デバイスに自動的に適用します。
...
* <Cmd sid="setting.server.syncOption.smartSync" product='2' /> オプションを選択すると、デバイスに保存されているユーザーのうちアクセスグループに属していないユーザーはサーバーで管理できません。このオプションを使用するには、<Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.device.root' /> → <Cmd sid='setting.menu.device.device' /> メニューで各デバイスを選択し、<Cmd sid="device.sync_all" product='2' /> をクリックして同期を実行します。 <Cmd sid="device.sync_all" product='2' /> 機能の詳細については <DocLink docId='platform/biostar_x/settings-device-using-functions' /> を参照してください。
...
2. カスタムユーザーフィールドの名前を入力し、タイプで希望の項目を選択します。
   ...
   * <Cmd sid="setting.server.custom_field.type.textbox" product='2' />：最大 32 文字の数字および文字を入力できます。

   * <Cmd sid="setting.server.custom_field.type.combobox" product='2' />：最大 20 個のサブ項目を 32 文字で追加でき、各項目はセミコロン（;）で区切ります。(例：選択1;選択2;選択3)
   ...
   * <Cmd sid='setting.server.custom_field.type.fileupload' product='2' />：ファイルアップロード用のフィールドを追加できます。
     ...
     * 許可されるファイルの拡張子に制限はありません。アップロードされたファイルは暗号化されたうえでデータベースに保存されます。
...
* ユーザー情報の登録・編集の詳細については <DocLink docId='platform/biostar_x/how-to-guide-user' /> を参照してください。

* カスタムユーザーフィールドを追加したユーザー情報をカードに印刷できます。詳細については <DocLink docId='platform/biostar_x/settings-card-printer#adding-custom-user-fields' /> を参照してください。
...
<details>
<summary>**NFCモバイルカード**をサポートするデバイスを表示</summary>

* モバイルデバイスのOS：Android 7.0 以上
...
<details>
<summary>**BLEモバイルカード**をサポートするデバイスを表示</summary>

* モバイルデバイスのOS：Android 7.0 以上 / iOS 13.0 以上
...
* <Cmd sid="setting.server.eventLogRetainDays" product='2' />：出入管理イベントログを保存する期間を設定します。
  ...
  入退室管理イベントログの保存期間の設定は、サーバーログにのみ適用されます。デバイスイベントログには、この設定が適用されません。デバイスイベントログを削除する場合は、<Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.device.root' /> → <Cmd sid='setting.menu.device.device' /> メニューでデバイスイベントログを個別に削除してください。
...
<Cmd sid='common.serverMatching' product='2' /> 機能は、**エッセンシャル**(Essential) ライセンス以上で追加オプションとして利用できます。ライセンスポリシーの詳細については、<DocLink docId='platform/biostar_x/licensing' />をご参照ください。
...
<Cmd sid="settings.expertSettings.enableOptionLabel" product='2' /> オプションを有効にする前に、<Cmd sid="setting.menu.server.server" /> メニューで行った変更がある場合は、先に変更内容を保存する必要があります。画面右下の <Cmd sid="button.apply" product='2' /> ボタンをクリックします。
...
* 登録済みの顔画像が2種類のテンプレートを持っていない場合、この機能を無効にできません。 <Cmd sid='team.menu.faceMigration' /> 機能の詳細については <DocLink docId='platform/biostar_x/migration-face' /> を参照してください。
