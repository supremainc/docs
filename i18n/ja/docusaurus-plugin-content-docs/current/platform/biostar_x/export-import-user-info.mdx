...
* **BioStar X** に追加していないカスタムユーザーフィールドがある場合、CSVファイルを正しくインポートできません。カスタムユーザーフィールドの追加方法については、<DocLink docId='platform/biostar_x/settings-server-server#custom-field' />を参照してください。
...
4. <Cmd sid='team.menu.export.csv' /> <Cmd sid='team.menu.export.title' /> ウィンドウが表示されたら、<Cmd sid='team.menu.export.exportReason' />(任意)を入力し、<Cmd sid='common.button.download' /> ボタンをクリックします。
...
<Cmd sid='team.menu.export.exportReason' /> は必須入力項目ではありません。エクスポート理由は <Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.auditTrail.root' /> → <Cmd sid='setting.menu.auditTrail.acAuditLog' /> で確認できます。
...
* CSVインポートでユーザーの顔を登録します。詳細については、<DocLink docId='platform/biostar_x/batch-import-face#import-csv' />を参照してください。

* CSVインポートでユーザーのPINを登録します。詳細については、<DocLink docId='platform/biostar_x/enroll-credential-pin' />を参照してください。

* 保存済みのCSVファイルに一致しない列を追加すると、**BioStar X** でファイルをインポートできません。

* ユーザー名の先頭が `&@~` で連続して始まる場合、該当ユーザーを CSV からインポートできません。

* 有効期限（失効日時）は 2037-12-31 23:59:59 まで設定できます。この日付を超える値を含む行はインポートに失敗します。

* エクスポートした CSV ファイルは、Excel などの表計算ソフトで直接開かないことを推奨します。ユーザー名が `=` 文字で始まる場合、プログラムが数式として認識し、実際の値と異なる表示になることがあります。
...
4. <Cmd sid='team.menu.dialog.exprotDataFile.title' /> ウィンドウで、データファイルを適用するデバイスを選択してください。

   <Image src='/img/biostarx-user-export-data-select-device.png' />

5. <Cmd sid='team.menu.export.exportReason' />(任意)を入力し、<Cmd sid='common.apply' /> ボタンをクリックします。

6. ローカルの任意のパスにデータファイルを保存します。
