...
id: add-sub-db
title: 補助データベースを追加する
description: マルチ通信サーバーを追加した際にメインサーバーの負荷を軽減するため、マルチ通信サーバーと連携する補助データベースを追加して接続する方法を説明します。
keywords:
  - データベース
  - 追加
  - 接続
...
マルチ通信サーバーを追加した際にメインサーバーの負荷を軽減するため、マルチ通信サーバーと連携する補助データベースを追加して接続する方法を説明します。

:::info

* 補助データベースを追加するには **マルチ通信サーバー** ライセンスが必要です。 ライセンスポリシーの詳細については、<DocLink docId='platform/biostar_x/licensing#features-add-ons' />を参照してください。
