...
**Entra ID** または **Active Directory** 設定機能は、**エッセンシャル**(Essential)ライセンス以上で追加オプションとして利用できます。ライセンスポリシーの詳細については、<DocLink docId='platform/biostar_x/licensing#features-add-ons' />を参照してください。

:::

## ディレクトリ連携の設定 {/* #intergratewithentraid */}
...
3. 画面左のサイドバーで <Cmd sid='setting.menu.system.root' /> → <Cmd sid='setting.menu.directoryIntegration.root' /> をクリックします。
...
Entra IDと連携するには、<Cmd sid='setting.menu.system.root' /> → <Cmd sid='setting.server' product='2' /> メニューで<Cmd sid='setting.userDevice.useridType' product='2' />項目を<Cmd sid='setting.userDevice.useridType.alphanumeric' product='2' />に設定する必要があります。
...
**BioStar X**の各ユーザーフィールドは、<strong>Entra ID</strong>のユーザー情報と一致する項目にマッピングできるようデフォルト設定されています。デフォルト設定されたフィールド値ではなく別のフィールド値を選択するには、<Cmd sid='settings.activeDirectory.FieldNameEI' product='2' /> 列の項目をクリックして、目的のフィールド値を選択します。
...
<Cmd sid='setting.menu.directoryIntegration.root' /> の設定をすべて完了したら、画面最下部の <Cmd sid='button.apply' product='2' /> ボタンをクリックして保存します。設定結果を確認するには、<DocLink inner='#checksettings' />を参照してください。
...
* Active Directoryと連携するには、<Cmd sid='setting.menu.system.root' /> → <Cmd sid='setting.server' product='2' />メニューで<Cmd sid='setting.userDevice.useridType' product='2' />項目を<Cmd sid='setting.userDevice.useridType.alphanumeric' product='2' />に設定してください。
...
* <Cmd sid='settings.activeDirectory.secureTransfer' product='2' />：Windows Active Directory サーバーと通信するときに暗号化を使用できます。 Active Directory 証明書サービスをインストールし、<Cmd sid='settings.activeDirectory.keystorePassword' product='2' />を設定するには、<DocLink docId='platform/biostar_x/ad-encrypt' />を参照してください。
...
**BioStar X**の各ユーザーフィールドは、<strong>Active Directory</strong>のユーザー情報と一致する項目にマッピングできるようデフォルト設定されています。デフォルト設定されたフィールド値ではなく別のフィールド値を選択するには、<Cmd sid='settings.activeDirectory.FieldNameAD' product='2' /> 列の項目をクリックして、目的のフィールド値を選択します。
...
## サポートされない文字の処理 {/* #unsupportedcharacter */}

**Entra ID**または**Active Directory**のユーザーIDまたはユーザー名に**BioStar X**でサポートされない文字が含まれる場合、システムはサポートされない文字のみを削除した値でユーザーを同期します。

> 例えば、ディレクトリサービスのユーザーIDが `john/doe` で、ユーザー名が `John<Doe>` の場合、**BioStar X**にはそれぞれ `johndoe`、`JohnDoe` として登録します。

:::info

* ユーザー名のピリオド(`.`)とアポストロフィ(`'`)はサポートされない文字から除外され、ディレクトリサービスの値のまま登録します。 

  > 例: `O'Brien`、`John.Doe` → ディレクトリサービスの値のまま登録

* ユーザーIDのピリオド(`.`)はサポートされない文字から除外され、ディレクトリサービスの値のまま登録します。ただし、アポストロフィ(`'`)はユーザーIDで使用できないため、削除した値で登録します。 

  > ディレクトリサービスのユーザーIDの例：
  >
  > * `O'Brien` → `OBrien` として登録 
  > * `john.doe` → `john.doe` のまま登録

* ユーザーIDおよび名前の入力規則について詳しくは、<DocLink docId='platform/biostar_x/add-user-basic-info' />を参照してください。

:::

## ディレクトリ連携の除外 {/* #excludeintegration */}
...
4. 選択したユーザーの詳細画面が表示されたら、<Cmd sid='common.advanced' product='2' /> セクションで<Cmd sid='user.adsync.exclude' product='2' />オプションを有効にしてください。
...
次に該当するユーザーは、自動的に連携から除外されます。

* ユーザー名の先頭が `&@~` の連続で始まる場合

* ユーザーIDまたはユーザー名の値が空の場合

* ユーザーID（最大32文字）またはユーザー名（最大48文字）の長さ制限を超える場合

* ユーザーIDが `admin` のように、システムで予約された値と同じ場合

* ユーザーIDがピリオドが一つ（`.`）または二つ（`..`）の場合

:::

## ディレクトリ連携の解除 {/* #disableIntegration */}

<Cmd sid='settings.activeDirectory.title' product='2' /> 機能を無効にする場合、次の手順に従います。

1. **BioStar X** に管理者 ID でログインします。

2. <Cmd sid='setting.menu.system.root' /> → <Cmd sid='setting.menu.directoryIntegration.root' /> をクリックします。
