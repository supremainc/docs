...
BioStation 3 Max、BioEntry W3、BioStation 3、FaceStation F2 は、顔テンプレートの形式がそれぞれ異なります。

* **NPU タイプ**：BioStation 3 Max、BioEntry W3、BioStation 3
...
* **顔テンプレートタイプが異なるデバイス間での同期所要時間**
  ...
  <Cmd sid='team.menu.faceMigration' /> を実行する前に、FaceStation F2 のみを接続して使用していた環境に BioStation 3 Max、BioEntry W3、BioStation 3 を追加して同期すると、テンプレート形式が異なるため画像から NPU タイプのテンプレートを抽出することになり、サーバーに登録されている顔の数によってはかなり時間がかかる場合があります。
...
* マイグレーションが成功したら、デバイスにユーザー情報を同期して変更を適用してください。 デバイスにユーザー情報を送信する方法の詳細については、<DocLink docId='platform/biostar_x/transfer-user-info-to-device' /> をご参照ください。
...
* ユーザーの顔画像をサーバーに保存せず、テンプレートのみを保存する方法の詳細については、<DocLink docId='platform/biostar_x/settings-server-server#storevisualfaceimage' /> をご参照ください。
