...
1. Press <Image src='/img/device/ico-wireless-info.svg' ico /> the wireless network you wish to use and then disable DHCP settings to allow users to modify the <Cmd>IP Address</Cmd>, <Cmd>Gateway</Cmd> and <Cmd>Subnet Mask</Cmd>.
...
1. To activate the device license, connect the USB with the license to the device and select <Image src='/img/device/ico-add.svg' ico />.
...
1. Change the desired item.
    ...
    * <Cmd>Date & Time</Cmd>: Check the current date and time. To modify it manually, disable <Cmd>Time Sync</Cmd>를 끄세요.
...
1. Select <Cmd>SETTINGS</Cmd> → <Cmd>DEVICE</Cmd> → <Cmd>Daylight Saving Time</Cmd> and then press <Image src='/img/device/ico-add.svg' ico /> to add additional daylight-saving time.
