...
keywords: [Bracket, Power, Network, Input, Relay, Standalone, Secure I/O, Wiegand]
---
...
1. Fix the bracket firmly using fixing screws through the bracket to the position where BioStation 3 will be installed.

    <Image src='/img/device/installation-device1-bs3.svg' className='none' alone />

    :::info

    * If installing BioStation 3 on a concrete wall, drill holes, insert PVC anchors, and fix them with fixing screws.
...
1. After assembling the cable cover, fix it firmly using the six screws on the cover.
...
1. Install BioStation 3 onto the fixed bracket.

    <Image src='/img/device/installation-device3-bs3.svg' className='none' alone />

1. Connect BioStation 3 to the bracket by rotating the product fixing screw of BioStation 3.
...
* Use the IEC/EN 62368-1 approved power adapter that supports higher power consumption than the product. If you wish to connect and use another device to the power supply adapter, you should use an adapter with a current capacity which is the same or larger than the total power consumption required for the terminal and another device.
    ...
    * Refer to the [Power](product-specifications#electrical) in the product specifications for maximum current consumption specifications.
...
BioStation 3 has an automatic MDI/MDIX function so that it can be connected to a PC directly using a normal straight type CAT-5 cable or a cross cable.
...
* Use a separate power supply for BioStation 3 and the door lock.
...
BioStation 3 can be connected to the door lock, door button, and door sensor directly without connecting a separate I/O device.
...
* BioStation 3 can be used as a multi-door controller with the slave devices with the RS-485 cable. The slave devices are used as dummy readers and authentication is performed in the master device.
...
Secure I/O 2 is an I/O device, can be connected to BioStation 3 with the RS-485 cable. Security can be maintained even if the connection between BioStation 3 and Secure I/O 2 has been lost or the power supply to BioStation 3 has been shut off due to external factors.
