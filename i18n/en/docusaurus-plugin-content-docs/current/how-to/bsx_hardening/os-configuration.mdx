...
1. Install all security and critical updates.
...
1. Reboot when required.

1. Verify update status — run again until "You're up to date" is displayed.
...
1. Review installed roles (e.g., File Services, IIS, Print Services).

1. Remove roles not required by BioStar X.

1. Only keep **.NET Framework Features** and **Desktop Experience** if needed by your admin workflow.

1. Open `services.msc` and set unneeded services (e.g., Fax, Remote Registry, Bluetooth, SMBv1) to **Disabled**.
...
1. Confirm **Manage settings** → **Real-time protection** is enabled.

1. Confirm **Cloud-delivered protection** and **Automatic sample submission** are enabled.

1. Keep Security intelligence (sometimes referred to as definitions) updated. Usually automatic via Windows Update.
...
1. Click **Turn on BitLocker** for each drive.

1. Choose authentication method: **TPM** (default) or **TPM + PIN** (recommended).

1. Save recovery key: **Active Directory** / **Azure AD** (preferred), or **secure vault** (do NOT store locally).

1. Choose encryption mode: **XTS-AES 256-bit** (recommended).
...
1. Configure the following **Account Policies** → **Password Policy**:
...
1. Configure the following **Account Policies** → **Account Lockout Policy**:

   * **Account lockout threshold**: 5 attempts

1. Disable or rename default **Administrator** account.

1. Require multi-factor authentication (MFA) for RDP or remote admin access.
...
1. Select the baseline for your OS version: **Windows Server 2019** or **2022 Security Baseline**.

1. Import baseline GPOs into **Group Policy Management Console (GPMC)**

   Open `gpmc.msc` → Right-click **Group Policy Objects** → Click **Import Settings**.

1. Link the baseline GPO to the **BioStar X server OU** in Active Directory (or apply locally if standalone).

1. Review critical settings: account lockout, audit logging, network security (SMB signing, LDAP signing, disabling NTLM fallback), LSA Protection.

1. **Test in staging first**, confirming BioStar X compatibility and then deploy to production.
