...
* **Card data structure**: Data structure required for the production of SCC (Secure Credential Card) and AOC (Access on Card)

* **Structure specification**: Detailed requirements for each field that must be complied with during card manufacturing
...
Smart cards produced according to this document operate normally on all BioStar compatible devices from Suprema.
