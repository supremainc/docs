...
BioStar Air and compatible devices must access specific network ports for seamless connection. Network ports allow secure communication between compatible devices, cloud services, and user applications. Administrators may need to verify a BioStar Air-compatible device's MAC address from its serial number for network configuration tasks.
