...
description: When adding a Multi Communications Server, add and connect an auxiliary database to reduce the load on the main server.
keywords:
  - Database
  - Add
  - Connect
...
When adding a Multi Communications Server, add and connect an auxiliary database to reduce the load on the main server.

:::info

* You need a **multi-communication server** license to add an auxiliary database. For more information on licensing policies, see <DocLink docId='platform/biostar_x/licensing#features-add-ons' />.
...
2. Click the <Cmd>DATABASE</Cmd> menu on the left sidebar.

3. Click <Cmd>+ Add Database</Cmd> in the upper right corner of the screen.
