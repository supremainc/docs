...
* <Cmd sid='door.dualAuthentication' product='2' />: You can set it so that two people (a general user and an administrator) must authenticate their credentials to activate the floor button.
  ...
  * To change dual authentication for devices set in restricted areas, modify the settings in the following menu: <Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.advancedAc.root' /> → <Cmd sid='zone.occupancy.title' product='2' />
