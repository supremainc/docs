...
This document provides guidance on how to enroll and set up an elevator. The elevator is used for floor control and forms part of the access control system along with the entrance door. Enrolling the elevator allows control over access permissions for specific floors.

## Before start {#before-start}

* Set the access level, access group, and floor level for access control before enrolling the elevator. Refer to the [following](#) for more details.

* Enroll the device before enrolling the elevator. For more information on device enrollment, refer to the following.
...
## Enroll elevator {#adding-elevator}

1. Click <Cmd sid='shell.header.setting' /> on the <Cmd sid='shell.header.launcher' /> page.

2. Click <Cmd>Elevator</Cmd> on the left sidebar.

3. Once the <Cmd sid='elevator.new.title' product='2' /> screen appears, set each section item in order.

    * <Cmd sid='common.information' product='2' />: Set the basic information for the elevator. 
    For more information, refer to the [following](#info).
...
4. Once all settings are complete, click <Cmd sid='button.apply' product='2' /> at the bottom of the screen.
...
* For more information on enrolling elevator groups, refer to the [following](settings-manage-elevators-group).

:::

### Set detailed information {#details}

In the <Cmd sid='common.detail' product='2' /> section, you can set the elevator's detailed information, such as devices connected to the elevator and floor information.
...
* Only master devices can be selected for the <Cmd sid='elevator.list.column.controller' product='2' /> option.
  ...
  BioEntry Plus, BioEntry W, and BioLite Net cannot be set as controllers.
...
* <Cmd sid='door.dualAuthentication' product='2' />: You can set it so that two people (a general user and an administrator) must authenticate their credentials to activate the floor button.
  ...
  * <Cmd sid='door.schedule' product='2' />: Select the schedule for using dual authentication. If your desired schedule is not available, click <Cmd sid='button.plusAddSchedule' product='2' /> to add one.
  ...
  * To change dual authentication for devices set in restricted areas, modify the settings in the following menu: <Cmd sid='shell.header.setting' /> → <Cmd>Advanced ACS</Cmd> → <Cmd sid='zone.occupancy.title' product='2' />.

    For more information on <Cmd sid='zone.occupancy.title' product='2' /> settings, refer to the [following](#).

  * For more information on scheduling settings, refer to the [following](settings-schedule).
