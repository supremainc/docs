...
This guide describes how to set scheduled lock zones. The scheduled lock zone provides a feature to lock the entrance door at specific times. This feature enhances access control and is useful for restricting access during certain periods.
...
6. Configure the details of the scheduled lock zone in the <Cmd sid='common.setting' product='2' /> section. For more information, refer to the [following](#setting).
...
9. Once all settings are complete, click <Cmd sid='button.apply' product='2' /> at the bottom of the screen.
...
* <Cmd sid='common.schedule' product='2' />: Select a schedule. If the desired schedule is not available, you can click <Cmd sid='button.plusAddSchedule' product='2' /> to add one.

:::info

* In <Cmd sid='zone.globalZone.local' product='2' /> mode, you can select multiple doors to configure a scheduled lock zone.

* For more details on schedule settings, refer to the [following](settings-schedule).
