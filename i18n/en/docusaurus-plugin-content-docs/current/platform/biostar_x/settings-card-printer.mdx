...
### Install and configure the cardPresso {/* #installing-configuring-cardPresso */}
...
2. Click <Cmd sid='setting.menu.credential.root' /> → <Cmd sid="setting.menu.cardPrinter.root" /> on the left sidebar.
...
3. In the <Cmd sid='setting.server.userDeviceManagement' product='2' /> section, click <Cmd sid="button.plusAdd" product='2' /> in <Cmd sid="setting.server.custom_field.title" product='2' /> to enter a <Cmd sid="setting.server.custom_field.configure.th.name" product='2' /> and select a <Cmd sid="setting.server.custom_field.configure.th.type" product='2' />. The order of the <Cmd sid="setting.server.custom_field.title" product='2' /> adds decides the element **ID** of the card template.

   <Image src='/img/biostarx-settings-card-printer-custom-user.png' />

   Based on the BioStar X element ID, **Order 1** is the same as **CUSTOM1**. For more information on BioStar X attribute IDs, see <DocLink inner='#card-template-attribute-ids' />.
