...
* If you change information for a user who is already registered on the device, you must resend the user information to reflect the changes on the device. For information on using the <Cmd sid='setting.server.automaticUserSync' product='2' /> option to synchronize user information with the device, see <DocLink docId='platform/biostar_x/settings-server-server#user-sync' />.
