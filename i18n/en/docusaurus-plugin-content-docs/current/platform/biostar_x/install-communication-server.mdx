...
* The communication server can only be installed on a 64-bit operating system. For more information on minimum system requirements, see <DocLink docId='platform/biostar_x/system-requirements#communication-server' />.

* A multi-communication server license is required to install and connect the communication server. For more information on licensing policies, see <DocLink docId='platform/biostar_x/licensing#features-add-ons' />.
...
5. When the <Cmd>Add Communication Server</Cmd> window appears, check the box for the item that matches the IP address of the installed communication server, and select the database to use from the <Cmd>Database</Cmd> column.
...
* To delete the communication server, check the box of the server to be deleted in the list and click <Cmd>Delete Communication Server</Cmd> in the upper right corner of the screen.

* When adding a communication server, you can also add a secondary database to reduce the load on the main server. For more information on adding a secondary database, see <DocLink docId='platform/biostar_x/add-sub-db' />.
