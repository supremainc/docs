...
The IP address entered in **Enroll Certificate** must be the same as the IP address set in **BioStar X**. You can check it in the menu path <Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.system.root' /> → <Cmd sid='setting.menu.server.server' /> under <Cmd sid='setting.server.biostarIPAdress' product='2' />.
