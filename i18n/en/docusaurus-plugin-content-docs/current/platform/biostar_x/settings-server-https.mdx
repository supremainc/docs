...
The IP address entered in **Enroll Certificate** must be the same as the IP address set in **BioStar X**. You can check it in the menu path <Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.server.root' /> → <Cmd sid='setting.menu.server.server' /> under <Cmd sid='setting.server.biostarIPAdress' product='2' />.
...
2. Click <Cmd sid='setting.menu.system.root' /> → <Cmd sid="setting.menu.server.https" /> on the left sidebar.
...
## Certificate installation for the VMS server {/* #vms-certification */}
...
* For more information on installing the certificate on the VMS server, see <DocLink docId='platform/biostar_x/settings-video-integration#install-vms-certification' />.

* The function to integrate with the VMS server must be purchased separately in an additional options package. For more information on the licensing policy, see <DocLink docId='platform/biostar_x/licensing' />.

:::

### Install the certificate on the client PC {/* #install-client-certification */}

1. On a **BioStar X** client PC, download the HTTPS certificate installer (*cert-register.zip*) from the login screen or <Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.system.root' /> → <Cmd sid="setting.menu.server.https" />.
