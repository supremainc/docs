...
* For more information on setting schedules, refer to the [following](settings-schedule).
...
* <Cmd sid='common.quickAction' product='2' /> can be set in <Cmd>Settings</Cmd> → <Cmd>Custom Interface</Cmd>. For more information about <Cmd sid='common.quickAction' product='2' />, refer to the [following](settings-custom-interface).
...
The <Cmd>Advanced Access Control</Cmd> option is only available with an **Advanced** or higher license. For more information on the licensing policy, refer to the [following](licensing).
...
* To set the email server information, click the <IcSet2 /> icon on the <Cmd>BioStar X</Cmd> tab. When the <Cmd sid='setting.triggerAction.smtpOption' product='2' /> window appears, enter the email server information and click the <Cmd sid='button.apply' product='2' /> button. For more information, refer to the [following](#).
...
After completing all settings, click <Cmd sid='button.apply' product='2' />.
