...
* Set access levels, access groups, and floor levels for access control before registering doors. For more information, see <DocLink docId='platform/biostar_x/settings-access-control' />.
...
4. When the <Cmd sid='door.addNewDoor' product='2' /> screen appears, set each section item in order.
   ...
   * <Cmd sid='common.information' product='2' />: Set the basic information of the door. For more information, see <DocLink inner='#info' />.

   * <Cmd sid='device.info.configuration' product='2' />: Set devices connected to the door, and entry and exit buttons along with door sensors. For more information, see <DocLink inner='#configdoor' />.

   * <Cmd sid='common.option' product='2' />: Set additional options. For more information, see <DocLink inner='#option' />.

   * <Cmd sid='door.antiPassBack' product='2' />: Set the anti-passback feature to manage access history and enhance security. For more information, see <DocLink inner='#antipassback' />.

   * <Cmd sid='timedAPB.title' product='2' />: Set the initialization time for the anti-passback feature. For more information, see <DocLink inner='#timed-apb' />.

   * <Cmd sid='door.alarm' product='2' />: You can set the alarm to trigger or block device usage when an anti-passback violation occurs. For more information, see <DocLink inner='#alarm' />.
...
* <Cmd sid='common.name' product='2' />: Enter the door name. Enter a name that can be specified for convenient management.
...
* For more information on registering door groups, see <DocLink docId='platform/biostar_x/settings-manage-door-group' />.

:::

### Set door settings {/* #configdoor */}
...
* <Cmd sid='common.doorRelay' product='2' />: Choose the [relay](../../reference/glossary#relay) that will function as the door lock mechanism.
...
* <Cmd sid='door.doorSensor' product='2' />: Choose the port to confirm the door's status. Setting it to <Cmd sid='common.none' product='2' /> will disable the use of the <Cmd sid='door.alarm' product='2' /> section.
  ...
  * <Cmd sid='door.apbUseDoorSensor' product='2' />: Set whether to use the door sensor when using the <Cmd sid='zone.apbUseDoorSensor' product='2' /> option.
  ...
  If <Cmd sid='timedAPB.title' product='2' /> is enabled, the <Cmd sid='door.apbUseDoorSensor' product='2' /> option cannot be used.
...
* <Cmd sid='common.open' product='2' />: Set options for door opening.
  ...
  * <Cmd sid='door.option.openCondition.useAutoDoor' product='2' />: If using an automatic door as the door, the relay can operate regardless of the door sensor's status. If set to <Cmd sid='door.openOnceOn' product='2' />, the <Cmd sid='door.openOnce' product='2' /> option cannot be used.
...
* <Cmd sid='door.dualAuthentication' product='2' />: Set to require two people (a regular user and an administrator) to authenticate credentials to open the door.
  ...
  * <Cmd sid='common.device' product='2' />: Select the device to use for dual authentication. To disable dual authentication, select <Cmd sid='door.dualAuthentication.approvalType.none' product='2' />.
  ...
  * <Cmd sid='door.authenticationTimeout' product='2' />: Set the waiting time between the first and second authentications. If the second authentication is not completed within the set time, the door will not open.

  :::note

  * Modify settings in the following menu to change the dual authentication of the device configured for occupancy limit settings. <Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.accessManagement.root' /> → <Cmd sid='setting.menu.advancedAc.root' /> → <Cmd sid='zone.occupancy.title' product='2' />

    For more information on the <Cmd sid='zone.occupancy.title' product='2' /> settings, see <DocLink docId='platform/biostar_x/settings-advanced-ac' />.

  * For more information on schedule settings, see <DocLink docId='platform/biostar_x/settings-schedule' />.
...
* <Cmd sid='door.authDoorControl.title' product='2' />: Enable the door to remain unlocked when an authorized user performs a Double Badge (Double Badge). For more information, refer to <DocLink docId='platform/biostar_x/settings-door-auth-control' />.

  :::note

  This setting is unavailable on the Add Door screen because the door is not in an access group. Save the door, add it to an access group, then set this on the Edit Door Information screen.
...
* This can be used when both entry and exit devices are installed, and setting <Cmd sid='door.exitDevice' product='2' /> option to <Cmd sid='common.none' product='2' /> in the <Cmd sid='device.info.configuration' product='2' /> section will render it unusable. For more information on setting up the exit device, see <DocLink inner='#configdoor' />.
...
In the <Cmd sid='door.alarm' product='2' /> section, you can configure to sound an alarm or block device usage when the door is forcibly opened, left open, or when an anti-passback violation occurs. Click <Cmd sid='button.addplus' product='2' /> for the item to add an alarm.
...
* <Cmd sid='door.antiPassBack' product='2' /> / <Cmd sid='timedAPB.title' product='2' />: Set alarm actions for anti-passback or timed anti-passback violations.
