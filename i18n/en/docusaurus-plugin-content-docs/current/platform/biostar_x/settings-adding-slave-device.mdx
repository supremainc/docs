...
5. When the list of slave devices connected to the master device appears, select the device to register. If the desired device is not listed, click <Cmd sid='button.search' product='2' /> in the upper right corner.

   <Image src='/img/biostarx-setting-device-adding-slave-list.png' />

6. Click <Cmd sid='button.add' product='2' />.

7. When the device registration confirmation message appears, click <Cmd sid='button.ok' product='2' />.

The registered slave device will appear under the selected device. In the device list, the master shows the <Mastermark /> icon, and the slave shows the <Slavemark /> icon.
