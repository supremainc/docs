...
Once the restore operation begins, the current data will revert to the backup state. It is recommended to create a backup of the current state before proceeding with the restore. To create a backup of the current state, refer to [the following](settings-system-system-backup).
