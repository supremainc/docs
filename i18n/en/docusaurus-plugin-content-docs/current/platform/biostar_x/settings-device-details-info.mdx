...
If you have made changes, click the <Cmd sid='button.apply' product='2' /> button. Changes will not be applied to the device if they are not saved.
...
For more information on creating a new device group, changing, or deleting group names, refer to [the following document](settings-manage-device-group).
...
For more information on upgrading the device's firmware, refer to [the following document](settings-device-upgrade-firmware).
...
* <Cmd sid='common.dst' product='2' />: You can apply the user's configured daylight saving time to the device. Refer to [the following document](#) to add a new daylight saving time.
