...
As shown in the image, you can link the configured area to a GIS map, allowing access in the order of area → facility → floor to monitor and control doors and cameras. This structure can also be seen in the sidebar of the <Cmd sid='arena.title' /> page.
...
If the area is linked to a GIS map, the map can be displayed in the <Cmd sid='arena.title' /> section.
...
* For more detailed information on how to configure the area linked with the GIS map, refer to [the following](settings-map-manage-area).
