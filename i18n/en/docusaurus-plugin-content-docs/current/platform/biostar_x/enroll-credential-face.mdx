...
You can use the import from CSV feature or enroll multiple users' faces at once, or send an email link to multiple users for them to enroll their faces directly on a mobile device. For more information, see <DocLink docId='platform/biostar_x/batch-import-face' />.

:::

## Enroll by device {/* #enrolldevice */}

Click <Cmd sid='button.addFace' product='2' /> in the <Cmd sid='common.credential' product='2' /> section. When the <Cmd sid='userCredential.enroll.visualFaceEnroll' product='2' /> window appears, set each item and click <Cmd sid='button.enroll' product='2' />.
...
* <Cmd sid='userCredential.enroll.useProfile' product='2' />: Select this option to use the enrolled face image as the profile image.
...
## Enroll with a webcam or photo {/* #erollbywebcambyphoto */}
