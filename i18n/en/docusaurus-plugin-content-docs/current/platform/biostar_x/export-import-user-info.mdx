...
keywords: [User Information, export, Import]
---

User information can be utilized from previous versions or different servers through export or import processes.
...
* If there are custom user fields not added to **BioStar X**, the CSV file cannot be imported correctly. Refer to [the following document](settings-server-server#user-and-device-management) for instructions on adding custom user fields.
...
1. Click the checkbox next to the users you want to save as a CSV file from the user list. You can select more than one user.

1. Click the <IcMoreW /> button in the top right corner of the screen, then select <Cmd sid='team.menu.export.title' /> → <Cmd sid='team.menu.export.csv' />.

1. Save the CSV file in your desired local path.
...
1. Click the <IcMoreW /> button in the top right corner of the screen, then select <Cmd sid='team.menu.import.title' /> → <Cmd sid='team.menu.import.csv' />.

1. Select the CSV file saved locally.

1. In the <Cmd sid='team.csv.title' /> window, set the <Cmd sid='team.csv.importStartRow' /> and click the <Cmd sid='common.button.continue' /> button.

    <Image src='/img/biostarx-user-import-01.png' />

1. User data fields from the CSV file will automatically map to user data fields in **BioStar X**. Click <Cmd sid='team.csv.reMapping' /> to remap fields with the same names.

    <Image src='/img/biostarx-user-import-02.png' />

1. Select whether to maintain existing user data or overwrite it with the CSV file information for users with an already registered user ID in **BioStar X**, then click <Cmd sid='common.button.continue' />.
...
* User facial data can be registered through CSV import. For more information, refer to [the following document](enroll-credential-face#import-csv).

* User PINs can be registered through CSV import. For more information, refer to [the following document](#) {/* PIN registration > CSV Import */}.
...
* Data exported from devices using outdated firmware cannot be imported into **BioStar X**. Always use the latest firmware version.
...
1. Click the checkbox next to the users you want to export from the user list. You can select more than one user.

1. Click the <IcMoreW /> button in the top right corner of the screen, then select <Cmd sid='team.menu.export.title' /> → <Cmd sid='team.menu.export.dataFile' />.

1. In the <Cmd sid='team.menu.export.dataFile.title' /> window, select the device to which the data file will be applied and click the <Cmd sid='common.button.ok' /> button.

1. Save the data file in your desired local path.
...
2. Click the <IcMoreW /> button in the top right corner of the screen, then select <Cmd sid='team.menu.import.title' /> → <Cmd sid='team.menu.import.dataFile' />.
