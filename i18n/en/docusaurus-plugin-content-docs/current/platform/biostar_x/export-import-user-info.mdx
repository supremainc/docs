...
* If there are custom user fields not added to **BioStar X**, the CSV file cannot be imported correctly. Refer to the following for how to add custom user fields.
...
* When importing user data, if credential data enrolled on the **BioStar X** server already exists, it will be overwritten by the existing data.
