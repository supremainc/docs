...
* If there are custom user fields not added to **BioStar X**, the CSV file cannot be imported correctly. For more information on adding custom user fields, see <DocLink docId='platform/biostar_x/settings-server-server#custom-field' />.
...
4. When the <Cmd sid='team.menu.export.csv' /> <Cmd sid='team.menu.export.title' /> window appears, enter <Cmd sid='team.menu.export.exportReason' /> (optional), and click <Cmd sid='common.button.download' />.
...
<Cmd sid='team.menu.export.exportReason' /> is optional. Check <Cmd sid='team.menu.export.exportReason' /> records in <Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.auditTrail.root' /> → <Cmd sid='setting.menu.auditTrail.acAuditLog' />.
...
6. Select whether to keep user data already registered with user IDs in **BioStar X** or to overwrite it with the information from the CSV file and click <Cmd sid='common.apply' />.
...
* User facial data can be enrolled through CSV import. For more information, see <DocLink docId='platform/biostar_x/batch-import-face#import-csv' />.

* User PINs can be enrolled through CSV import. For more information, see <DocLink docId='platform/biostar_x/enroll-credential-pin' />.

* If you add columns that do not match the saved CSV file, you cannot import the file into **BioStar X**.

* If a username starts with `&@~`, importing that CSV entry fails.

* Set the validity period (expiration date and time) up to 2037-12-31 23:59:59. Import fails for entries with values after this date.

* Avoid opening the exported CSV file directly in spreadsheet programs such as Excel. If a username starts with the `=` character, the program may treat it as a formula and show a different value.
...
4. In the <Cmd sid='team.menu.dialog.exprotDataFile.title' /> window, select the device to apply the data file.

   <Image src='/img/biostarx-user-export-data-select-device.png' />

5. Enter <Cmd sid='team.menu.export.exportReason' /> (optional) and click <Cmd sid='common.apply' />.

6. Save the data file in your desired local path.
...
To import file exported from **BioStar X** into devices, you must rename the file as follows so that the device can recognize it. The file name format will be adjusted to be automatically recognized through future device firmware updates.
