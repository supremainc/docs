...
5. Save the data file in your desired local path. The file name is generated in the format `BioStarX_YYYYMMDD_HHMMSS.tgz`.
...
To import files exported from **BioStar X** into devices, you must rename the file as follows so that the device can recognize it. The file name format will be adjusted to be automatically recognized through future device firmware updates.
