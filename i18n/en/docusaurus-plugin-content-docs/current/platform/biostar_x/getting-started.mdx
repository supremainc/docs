...
keywords: [Installation Guide, MariaDB, SQL Server, Database]
---
...
* Do not install **BioStar X** on a PC where **BioStar 1** is installed. This may cause performance problems.
...
* Upgrading directly from the existing version to the latest version is possible from **BioStar 2.6.0** or higher. To upgrade to the latest version from **BioStar 2.6.0** or below, install in the following order.
...
* If you are using Windows 8.1, please refer to the following link to install the KB2919355 update. Link 1

* If you are using MS SQL 2012 Express, please refer to the following link to install the Service Pack 3. [Link 2](https://www.microsoft.com/download/details.aspx?id=49996)

* If you are using MS SQL 2014 Express, please refer to the following link to install Service Pack 2. [Link 3](https://www.microsoft.com/en-us/download/details.aspx?id=53168)

* If MS SQL Server and **BioStar X** are installed on different PCs, you need to install the Native Client on the PC where **BioStar X** is installed by referring to the following link. [Link 4](https://www.microsoft.com/en-us/download/details.aspx?id=50402)
...
* If you want to back up the database of **BioStar X**, be sure to also back up the *enckey* in the *\Program Files\BioStar 2 (x64)\util* folder and the *system.conf* and *setting.conf* file in the *\Program Files\BioStar 2(x64)* folder. Otherwise, the database will be unavailable.
...
* The default values for the ports used by **BioStar X** are as follows. If another program occupies the same port, **BioStar X** may not work properly.
  ...
  * T&A Cloud Port: 52001

  * Unified Gateway HTTP Port: 5000

  * Report HTTP Port: 4213
...
If you are using a user-configured database, please check the items below before installing **BioStar X**.
...
In environments with many transactions, please backup the logs periodically to ensure that the size of the log files does not increase.
...
If you are using **Microsoft Windows Active Directory**, please complete the presets as below before setting up the Windows Authentication database.
...
* You can quickly find out how to install **BioStar X** in the [following document](tutorial-express-install).
