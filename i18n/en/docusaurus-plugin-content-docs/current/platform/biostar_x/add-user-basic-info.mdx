...
* <Cmd sid='common.group' product='2' />: Select the user's group. For more information about adding and managing user groups, refer to [this document](user-group-manage).
...
* The values that can be set for <Cmd sid='team.user.table.id' /> may vary depending on the <Cmd>User ID Type</Cmd> option in the <Cmd sid='shell.header.setting' /> → <Cmd>Server</Cmd> menu. For more information, refer to [this document](settings-server-server#user-and-device-management).
...
* For more information about adding the <Cmd sid='team.userTemplate.table.customField' />, refer to [this document](settings-server-server#user-and-device-management).
