...
* The values that can be set for <Cmd sid='team.user.table.id' /> may vary depending on the <Cmd>User ID Type</Cmd> option in the <Cmd sid='shell.header.setting' /> → <Cmd>Server</Cmd> menu. For more information, refer to the [following](settings-server-server#user-and-device-management).
