...
Devices connected to **BioStar X** can easily upgrade firmware without additional connections or operations. Keeping the device firmware up to date resolves security vulnerabilities and allows access to new features.

## Before you start {/* #before-start */}

Before upgrading the device firmware, prepare the new version of the firmware file. Save the downloaded firmware file to the path below and extract it. Create a new folder if the *firmware* folder does not exist.
...
* Before upgrading firmware, make sure the device has power and network connectivity. In particular, make sure power is not interrupted during the upgrade.

* Depending on the device type and mode, it may take up to 1 hour or longer. Also, the device cannot be used during the upgrade.

* Do not turn off the device during the firmware upgrade. When the upgrade is complete, the device restarts automatically.
...
4. Click <Cmd sid='button.firmwareUpgrade' product='2' /> in the <Cmd sid='device.info.information' product='2' /> section of the device details screen.
...
6. When the warning message window appears, click <Cmd sid='button.continue' product='2' />.
...
4. Click <Cmd sid='button.firmwareUpgrade' product='2' /> at the top right of the device list.
