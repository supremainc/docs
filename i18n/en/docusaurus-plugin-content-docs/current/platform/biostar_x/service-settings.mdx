...
## Situations that require changing settings {/* #when-to-change */}
...
4. To apply changes, click <Cmd>Apply</Cmd> in the upper right corner of the screen.
...
* Change the value of <Cmd>Client Port</Cmd>, then manually restart the **BioStar X Coordinator Service** in **Windows Services**(*services.msc*).

* After restarting the **BioStar X Coordinator Service**, go to **BioStar X Service Manager** → **SERVICES** and restart all services. 

  Click <Cmd>Stop All</Cmd> in the top right corner of the screen. After all services are stopped, when the <Cmd>Start All</Cmd> button is enabled, click the button.
