...
* La función de integración de VMS está disponible como opción adicional para licencias superiores a **Avanzado**. Para obtener más información sobre las políticas de licencia, consulte <DocLink docId='platform/biostar_x/licensing#features-add-ons' />.
...
1. Ve a la ruta *C:\Program Files\BioStar X\third* y ejecuta la consola de comandos.
   ...
   Si el VMS está instalado en un servidor distinto de **BioStar X**, copia el archivo *scaleUtil-1.0-all.jar* al servidor VMS y sigue los pasos que aparecen a continuación.
...
2. Ejecuta el siguiente comando. 
   ...
   * Reemplaza la sección `<%VENDOR.NAME%>` con el nombre del producto de tu VMS.

   * Reemplaza la sección `{VMS_IP_ADDRESS}` con la dirección IP del servidor VMS.
...
2. Descargue el instalador del certificado HTTPS (*cert-register.zip*) desde la pantalla de inicio de sesión o desde <Cmd sid='shell.header.setting' /> → <Cmd sid='setting.menu.server.root' /> → <Cmd sid="setting.https.https" product='2' />.

3. Descomprima el archivo descargado y ejecute el archivo **cert-register.exe**. Aparecerá la ventana **Enrollment Certification**.
...
5. Haga clic en el botón **Enrollment**.
