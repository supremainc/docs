...
Ofrece métodos de desarrollo reales y ejemplos de implementación de conectores integrados con **BioStar X**.

## Implementación de endpoint requerida

Para desarrollar un conector integrado con **BioStar X**, debe implementar los siguientes dos endpoints. Este documento utiliza `/bsx` como ruta de ejemplo.

### Endpoint de registro de conector

<Badge className='get' only>GET</Badge> `/bsx`

Este es el endpoint que el **BioStar X Service Manager** llama para comprobar la accesibilidad del servidor al registrar el conector.
...
<Badge className='post' only>POST</Badge> `/bsx`

Este es el endpoint que se llama cuando el usuario hace clic en el ícono del conector en la interfaz web de **BioStar X**. En este proceso, se solicita la sesión del usuario conectado en **BioStar X** y se recibe una respuesta. 

:::info

Se requiere un certificado para la comunicación segura entre **BioStar X** y el conector. El certificado puede descargarse desde la página de detalles del conector después del registro. Para obtener más información, consulte [lo siguiente](registration-guide#register-plugin).
