...
옵션:
  ...
  --output    출력 파일 경로 (기본값: "./cover.html")
    ...
    const { isKorean, isJapanese, isSpanish, isRussian, langCover } = detectLanguage(lang, number);
    const processedSubtitle = getLocalizedSubtitle(args.subtitle || 'IG', isKorean, isJapanese, isSpanish, isRussian);
    const { processedVersion } = getProcessedVersionAndNumber(version, number, isKorean, isJapanese, isSpanish, isRussian);
